  ...
  return [
    {
      ...
      message:
        "Points if it is a fruit, vegetables, dairy, protein, or whole grain.",
    },
    {
      firstIcon: <FontAwesomeIcon icon={faInfoCircle} />,
...
export default class SnackDetailsComponent extends Component {
  ...
  fetchSnackUSDA() {
    ...
    fetchSnackByIDUSDA(this.props.match.params.snack_id)
  ...
  calculate() {
    ...
    for (
      let index = 0;
      index < this.state.snack.foodNutrients.length;
      index++
    ) {
      /* Calories - Energy */
    ...
    if (score.calories !== 0) {
      score.totalFat =
        ((searchedFat * score.gRatio * 9) / score.calories) * 100;
    } else {
      score.totalFat = 0;
    }
    ...
    if (score.calories !== 0) {
      score.satFat =
        ((searchedSatFat * score.gRatio * 9) / score.calories) * 100;
    } else {
      score.satFat = 0;
    }
  ...
  processScoreFeedBack(score) {
    ...
    switch (true) {
      case score < 6:
        return feedbackJSON.scoreFeedback.feedback1to5[
          Math.floor(
            Math.random() * feedbackJSON.scoreFeedback.feedback1to5.length
          )
        ];
      case score >= 6 && score < 8:
        return feedbackJSON.scoreFeedback.feedback6to7[
          Math.floor(
            Math.random() * feedbackJSON.scoreFeedback.feedback6to7.length
          )
        ];
      case score >= 8 && score < 10:
        return feedbackJSON.scoreFeedback.feedback8to9[
          Math.floor(
            Math.random() * feedbackJSON.scoreFeedback.feedback8to9.length
          )
        ];
      default:
        return feedbackJSON.scoreFeedback.feedback10to11[
          Math.floor(
            Math.random() * feedbackJSON.scoreFeedback.feedback10to11.length
          )
        ];
    }
  }
  ...
  processNutritionFeedback(score) {
    ...
    if (
      this.state.score.totalScore === 0 ||
      this.state.score.totalScore === 0.0
    ) {
      name = "calScore";
    ...
    switch (true) {
      case name === "calScore":
        return feedbackJSON.nutritionFeedback.calScore[
          Math.floor(
            Math.random() * feedbackJSON.nutritionFeedback.calScore.length
          )
        ];
      case name === "fatScore":
        return feedbackJSON.nutritionFeedback.fatScore[
          Math.floor(
            Math.random() * feedbackJSON.nutritionFeedback.fatScore.length
          )
        ];
      case name === "satScore":
        return feedbackJSON.nutritionFeedback.satScore[
          Math.floor(
            Math.random() * feedbackJSON.nutritionFeedback.satScore.length
          )
        ];
      case name === "tranScore":
        return feedbackJSON.nutritionFeedback.tranScore[
          Math.floor(
            Math.random() * feedbackJSON.nutritionFeedback.tranScore.length
          )
        ];
      case name === "sodiumScore":
        return feedbackJSON.nutritionFeedback.sodiumScore[
          Math.floor(
            Math.random() * feedbackJSON.nutritionFeedback.sodiumScore.length
          )
        ];
      case name === "processedScore":
        return feedbackJSON.nutritionFeedback.processedScore[
          Math.floor(
            Math.random() * feedbackJSON.nutritionFeedback.processedScore.length
          )
        ];
      case name === "sugarScore":
        return feedbackJSON.nutritionFeedback.sugarScore[
          Math.floor(
            Math.random() * feedbackJSON.nutritionFeedback.sugarScore.length
          )
        ];
      default:
      ...
      <Container className="mt-4">
            ...
            <Card>
              <Card.Body>
                ...
                <Form.Group style={{ width: "300px", margin: "10px auto" }}>
                  <Form.Label>Portion Size (in {this.state.unit}):</Form.Label>
                  <Row>
                    <Col xs={12} md={8}>
                      <Form.Control
                        type="number"
                        placeholder={`${this.state.portion} ${
                  ...
                  <div>
                    <Button variant="link" onClick={this._toggle}>
                      Not sure how much?
                    </Button>
                  </div>
                </Form.Group>
        ...
        <Row
          ...
          style={{
            width: "1350px",
            margin: "30px auto",
            display: this.state.showResults ? "" : "none",
          }}
        >
          <Col xs={24} md={10}>
            <Card>
              <Card.Body>
                <Card.Title>
                  <center>
                    <p>
                      <strong>Score Breakdown for {this.snack}</strong>
                    </p>
                  </center>
                </Card.Title>
